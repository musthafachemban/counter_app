...
function Counter() {
  ...
  // hook to dispatch  afunction in action 
  const dispatch=useDispatch()
  // component can access the stae by using useSelector Hook
  ...
  return (
    <>
      <div className='d-flex align-items-center justify-content-center w-100 mt-5 flex-column'>
          ...
          <div>
            <input onChange={(e)=>setRange(e.target.value)} type="text" className='form-control w-100 mt-3' placeholder='enter a range' style={{borderColor:'blue'}} />
          </div>
      </div>
    </>
  )
